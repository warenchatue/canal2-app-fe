export default defineEventHandler(async (event) => {
  ...
  if (action == 'get') {
    ...
    return {
      total: data.debts.data.length,
      header: data.debts.headers,
      meta: data.debts.meta,
      data: filterData(data.debts.data, filter, page, perPage),
    }
  } else if (action == 'post') {
    ...
    const tontines = data.filter(
      (d) => d.name == body.username && d.due == body.password,
    )
...
function filterData(
  ...
  return data
    .filter((item) => {
      return [item[0], item[1], item[2]].some((item) => item.match(filterRe))
    })
...
async function getLoans() {
  ...
  const data: any = await $fetch(
    baseUrl + 'loansSummaries/generalLoansReport',
    {
      method: 'get',
      headers: {
