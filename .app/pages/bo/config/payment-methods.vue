...
const isModalNewPaymentMethodOpen = ref(false)
const isModalDeletePaymentMethodOpen = ref(false)
...
function editPaymentMethod(paymentMethod: any) {
  isModalNewPaymentMethodOpen.value = true
  isEdit.value = true
  setFieldValue('paymentMethod._id', paymentMethod._id)
  setFieldValue('paymentMethod.label', paymentMethod.label)
  setFieldValue('paymentMethod.description', paymentMethod.description)
}

function selectPaymentMethod(tax: any) {
  currentPaymentMethod.value = tax
  expanded.value = false
}

function confirmDeletePaymentMethod(tax: any) {
  isModalDeletePaymentMethodOpen.value = true
  isEdit.value = false
  currentPaymentMethod.value = tax
}
...
async function deletePaymentMethod(paymentMethod: any) {
  ...
  const response = await useFetch('/api/accountancy/payment-methods', {
    method: 'delete',
  ...
  if (response.data?.value?.success) {
    ...
    toaster.show({
      title: 'Success',
      message: `Moyen de paiment supprimée !`,
      color: 'success',
      icon: 'ph:check',
      closable: true,
    })
    isModalDeletePaymentMethodOpen.value = false
    filter.value = 'paymentMethod'
    filter.value = ''
  } else {
    toaster.clearAll()
    toaster.show({
    ...
    try {
      ...
      if (isSuccess) {
        ...
        toaster.show({
          ...
          message:
            isEdit.value == false
              ? `Moyen de paiment  créé !`
              : `Moyen de paiment  mis à jour`,
          color: 'success',
          icon: 'ph:check',
          closable: true,
        })
        isModalNewPaymentMethodOpen.value = false
        resetForm()
        filter.value = 'tax'
        filter.value = ''
      } else {
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        ...
        <BaseButton
          @click=";(isModalNewPaymentMethodOpen = true), (isEdit = false)"
          color="primary"
              ...
              <TairoTableRow v-for="item in data?.data" :key="item.id">
                ...
                <TairoTableCell spaced>
                  <div class="flex">
                    <BaseButtonAction
                      class="mx-2"
                      @click.prevent="selectPaymentMethod(item)"
                      muted
                    >
                    ...
                    /></BaseButtonAction>
                    <BaseButtonAction
                      @click="confirmDeletePaymentMethod(item)"
                      :disabled="
                        authStore.user.appRole.name != UserRole.superAdmin
                      "
                      class="mx-2"
    ...
    <!-- Modal new paymentMethod -->
    <TairoModal
      :open="isModalNewPaymentMethodOpen"
      size="xl"
      @close="isModalNewPaymentMethodOpen = false"
    >
      <template #header>
        <!-- Header -->
        <div class="flex w-full items-center justify-between p-4 md:p-6">
          ...
          >
            {{ isEdit == true ? 'Mise à jour' : 'Nouvelle' }} Méthode de
            paiement
          </h3>

          <BaseButtonClose @click="isModalNewPaymentMethodOpen = false" />
        </div>
      </template>
      ...
      <template #footer>
        ...
        <div class="p-4 md:p-6">
          <div class="flex gap-x-2">
            <BaseButton @click="isModalNewPaymentMethodOpen = false"
              >Annuler</BaseButton
            >
    ...
    <!-- Modal delete -->
    <TairoModal
      :open="isModalDeletePaymentMethodOpen"
      size="sm"
      @close="isModalDeletePaymentMethodOpen = false"
    >
      <template #header>
        <!-- Header -->
        <div class="flex w-full items-center justify-between p-4 md:p-6">
          ...
          <BaseButtonClose @click="isModalDeletePaymentMethodOpen = false" />
        </div>
      </template>
      ...
      <div class="p-4 md:p-6">
        <div class="mx-auto w-full max-w-xs text-center">
          ...
          >
            Supprimer
            <span class="text-red-500">{{ currentPaymentMethod?.name }}</span> ?
          </h3>
      ...
      <template #footer>
        ...
        <div class="p-4 md:p-6">
          <div class="flex gap-x-2">
            <BaseButton @click="isModalDeletePaymentMethodOpen = false"
              >Annuler</BaseButton
            >
            ...
            <BaseButton
              color="primary"
              flavor="solid"
              @click="deletePaymentMethod(currentPaymentMethod)"
              >Suppimer</BaseButton
            >
          </div>
        </div>
      ...
      <div class="relative flex w-full flex-col px-8">
        ...
        <div v-else class="mt-8" @click.>
          <div class="flex items-center justify-center">
            <BaseAvatar
              :src="currentPaymentMethod?.logo"
              :text="currentPaymentMethod.initials"
          ...
          <div class="text-center">
            ...
            <BaseParagraph size="sm" class="text-muted-400">
              <span>{{ currentPaymentMethod?.description }}</span>
            </BaseParagraph>
            <div class="my-4">
              <BaseParagraph
                size="sm"
