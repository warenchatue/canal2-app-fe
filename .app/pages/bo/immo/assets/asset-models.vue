...
definePageMeta({
  ...
  preview: {
    title: 'Modeles',
    description: 'Gestion des modeles',
    categories: ['bo', 'immo', 'asset-models'],
...
const isModalNewAssetModelOpen = ref(false)
...
const zodSchema = z
  ...
  .superRefine((data, ctx) => {
    if (!data.assetModel.code) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: VALIDATION_TEXT.CODE_REQUIRED,
        path: ['assetModel.code'],
      })
    }
  })
...
function editAssetModel(assetModel: any) {
  isModalNewAssetModelOpen.value = true
  isEdit.value = true
  setFieldValue('assetModel._id', assetModel._id)
  setFieldValue('assetModel.code', assetModel.code)
  setFieldValue('assetModel.name', assetModel.name)
  setFieldValue('assetModel.description', assetModel.description)
}

function selectAssetModel(assetModel: any) {
  currentAssetModel.value = assetModel
  expanded.value = false
}

function confirmDeleteAssetModel(assetModel: any) {
  isModalDeleteAssetModelOpen.value = true
  isEdit.value = false
  currentAssetModel.value = assetModel
}

async function deleteAssetModel(assetModel: any) {
  ...
  const response = await useFetch('/api/immo/assets/asset-models', {
    method: 'delete',
  ...
  if (response.data?.value?.success) {
    ...
    toaster.show({
      title: 'Success',
      message: `Modele supprimée !`,
      color: 'success',
      icon: 'ph:check',
      closable: true,
    })
    isModalDeleteAssetModelOpen.value = false
    filter.value = 'assetModel'
    filter.value = ''
  } else {
    toaster.clearAll()
    toaster.show({
...
const onSubmit = handleSubmit(
  async (values) => {
    ...
    console.log('assetModel-create-success', values)
    ...
    try {
      ...
      if (isSuccess) {
        ...
        toaster.show({
          title: 'Success',
          message:
            isEdit.value == false ? `Modele créé !` : `Modele mis à jour`,
          color: 'success',
          icon: 'ph:check',
          closable: true,
        })
        isModalNewAssetModelOpen.value = false
        resetForm()
        filter.value = 'assetModel'
        filter.value = ''
      } else {
  ...
  (error) => {
    ...
    console.log('assetModel-create-error', error)
  ...
  <div>
    <TairoContentWrapper>
      ...
      <template #right>
        ...
        <BaseButton
          @click=";(isModalNewAssetModelOpen = true), (isEdit = false)"
          color="primary"
              ...
              <TairoTableRow v-for="item in data?.data" :key="item.id">
                ...
                <TairoTableCell spaced>
                  <div class="flex">
                    <BaseButtonAction
                      class="mx-2"
                      @click.prevent="selectAssetModel(item)"
                      muted
                    >
                    ...
                    <BaseButtonAction
                      :disabled="
                        ...
                        authStore.user.appRole.name != UserRole.superAdmin
                      "
                      @click="editAssetModel(item)"
                    >
                      <Icon name="lucide:edit" class="h-4 w-4"
                    /></BaseButtonAction>
                    <BaseButtonAction
                      @click="confirmDeleteAssetModel(item)"
                      :disabled="
                        authStore.user.appRole.name != UserRole.superAdmin
                      "
                      class="mx-2"
    ...
    <!-- Modal new Modele -->
    <TairoModal
      :open="isModalNewAssetModelOpen"
      size="xl"
      @close="isModalNewAssetModelOpen = false"
    >
      <template #header>
        <!-- Header -->
        <div class="flex w-full items-center justify-between p-4 md:p-6">
          ...
          >
            {{ isEdit == true ? 'Mise à jour' : 'Nouvelle' }} Modele
          </h3>

          <BaseButtonClose @click="isModalNewAssetModelOpen = false" />
        </div>
      </template>
      ...
      <template #footer>
        ...
        <div class="p-4 md:p-6">
          <div class="flex gap-x-2">
            <BaseButton @click="isModalNewAssetModelOpen = false"
              >Annuler</BaseButton
            >
    ...
    >
      ...
      <div class="p-4 md:p-6">
        <div class="mx-auto w-full max-w-xs text-center">
          ...
          >
            Supprimer
            <span class="text-red-500">{{ currentAssetModel?.name }}</span> ?
          </h3>
        ...
        <div class="p-4 md:p-6">
          <div class="flex gap-x-2">
            ...
            <BaseButton
              color="primary"
              flavor="solid"
              @click="deleteAssetModel(currentAssetModel)"
              >Suppimer</BaseButton
            >
          </div>
        </div>
      ...
      <div class="relative flex w-full flex-col px-8">
        ...
        <div v-else class="mt-8" @click.>
          <div class="flex items-center justify-center">
            <BaseAvatar
              :src="currentAssetModel?.logo"
              :text="currentAssetModel.initials"
          ...
          <div class="text-center">
            <BaseHeading tag="h3" size="lg" class="mt-4">
              <span>{{ currentAssetModel?.name }}</span>
            </BaseHeading>
            <BaseParagraph size="sm" class="text-muted-400">
              <span>{{ currentAssetModel?.description }}</span>
            </BaseParagraph>
            <div class="my-4">
              <BaseParagraph
                size="sm"
