...
definePageMeta({
  title: "B.C & Factures F'ss",
  preview: {
    title: "B.C & Factures F'ss",
    description: 'Gestion des bon de commande et factures',
...
const { data: allOrders } = await useFetch('/api/purchases/orders', {
  query,
})

const { data: suppliers, pending: pendingAnnouncer } = await useFetch(
...
if (pageType.value == 'view' || pageType.value == 'edit') {
  if (pageValue.value == 'order') {
    pageTitle = 'Mise à jour Bon de Commande'
  } else if (pageValue.value == 'invoice') {
  ...
  if (pageValue.value == 'order') {
    const { data: singleOrder } = await useFetch('/api/purchases/orders', {
      query,
    })
    ...
    const { data: singleOrder } = await useFetch('/api/purchases/invoices', {
      query,
    })
    if (singleOrder.value?.success) {
      currentOrderInvoice.value = singleOrder.value?.data
...
async function refreshData() {
  ...
  if (pageValue.value == 'order') {
    ...
    const singleOrder = await $fetch(
      '/api/purchases/orders?action=findOne&token=' +
        token.value +
        '&id=' +
        pageId.value,
      {
    ...
    // })
    const singleOrder = await $fetch(
      '/api/purchases/invoices?action=findOne&token=' +
        token.value +
        '&id=' +
        pageId.value,
      {
...
async function addInvoicePayment() {
  ...
  const response = await useFetch('/api/purchases/invoices', {
    ...
    body: {
      ...
      supplier: currentOrderInvoice?.value?.supplier._id,
      data: {
...
function editOrderInvoiceFile(currentOrderInvoice: any, isFromOrder = false) {
  ...
  setTimeout(() => {
    ...
    setFieldValue('order.supplier', {
      id: currentOrderInvoice.supplier._id,
      name: currentOrderInvoice.supplier.name,
    })
    if (isFromOrder == false) {
    ...
    try {
        ...
        if (isEdit.value == true) {
          ...
          const response = await useFetch('/api/purchases/orders', {
            ...
            body: {
              ...
              supplier: values.order?.supplier?.id,
      ...
      if (isSuccess) {
        ...
        if (isEdit.value == true) {
          ...
          if (pageValue.value == 'order') {
            router.push(
              '/bo/purchases/orders/edit-order-' + orderInvoiceId.value,
            )
          } else {
            router.push(
              '/bo/purchases/orders/edit-invoice-' + orderInvoiceId.value,
            )
          }
        }
      } else {
          ...
          <div class="mb-4 flex items-center justify-between">
            ...
            <div class="flex items-center justify-end gap-3">
              ...
              <BaseButtonIcon
                ...
                "
                :to="
                  '/bo/purchases/orders/edit-order-' + currentOrderInvoice?._id
                "
                condensed
                class="!w-32"
                shape="xl"
              ...
              <BaseButtonIcon
                ...
                "
                :to="
                  '/bo/purchases/orders/edit-invoice-' +
                  currentOrderInvoice?._id
                "
                condensed
                class="!w-32"
                shape="xl"
                  ...
                  <div>
                    <div class="flex items-center gap-3">
                      <div class="">
                        ...
                        >
                          {{ currentOrderInvoice?.supplier?.name }}
                        </BaseParagraph>
                      </div>
                    </div>
                    <div class="flex gap-2 mt-2">
                      ...
                      >
                        <p
                          v-if="currentOrderInvoice?.supplier?.address"
                        ...
                        <p
                          v-if="currentOrderInvoice?.supplier?.phone"
                          class="text-muted-700 dark:text-muted-100 mt-1 text-[9px] font-bold"
                        >
                          Mobile :
                        </p>
                        <p
                          v-if="currentOrderInvoice?.supplier?.email"
                          class="text-muted-700 dark:text-muted-100 mt-1 text-[9px] font-bold"
                        >
                          Courriel :
                        </p>
                        ...
                        <p
                          v-if="currentOrderInvoice?.supplier?.rc"
                          class="text-muted-700 dark:text-muted-100 mt-1 text-[9px] font-bold"
                        >
                          R/C :
                        </p>
                        <p
                          v-if="currentOrderInvoice?.supplier?.nc"
                          class="text-muted-700 dark:text-muted-100 mt-1 text-[9px] font-bold"
                        >
                          N. Contribuable :
                        </p>
                        <p
                          v-if="currentOrderInvoice?.supplier?.niu"
                          class="text-muted-700 dark:text-muted-100 mt-1 text-[9px] font-bold"
                        >
                      ...
                      >
                        <p class="text-[9px]">
                          {{ currentOrderInvoice?.supplier?.address }}
                        </p>
                        <p class="mt-1 text-[9px]">
                          {{ currentOrderInvoice?.supplier?.phone }}
                        </p>
                        <p class="mt-1 text-[9px]">
                          {{ currentOrderInvoice?.supplier?.email }}
                        </p>
                        <p class="mt-1 text-[9px]">
                          {{ currentOrderInvoice?.supplier?.rc }}
                        </p>
                        <p class="mt-1 text-[9px]">
                          {{ currentOrderInvoice?.supplier?.nc }}
                        </p>
                        <p class="mt-1 text-[9px]">
                          {{ currentOrderInvoice?.supplier?.niu }}
                        </p>
                      </div>
                    </div>
                  </div>
    ...
    >
      <template #header>
        ...
        <div class="flex w-full items-center justify-between p-4 md:p-6">
          ...
          >
            Confirmation
            {{ pageValue == 'order' ? 'du bon de commande' : 'de la facture' }}
          </h3>
      ...
      <div class="p-4 md:p-6">
        <div class="mx-auto w-full max-w-xs text-center">
          ...
          >
            ...
            {{ pageValue == 'order' ? 'le bon de commande' : 'la facture' }}
